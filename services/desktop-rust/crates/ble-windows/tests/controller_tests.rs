use ble_windows::controller::{
    WinBleError, WinRtBleAvailability, WinRtBleCentralController, WinRtBleGateway,
};
use transport_ble::{
...
#[derive(Default)]
struct FakeGateway {
    availability: WinRtBleAvailability,
    operations: Vec<String>,
    writes: Vec<(String, Vec<u8>)>,
}

#[async_trait::async_trait]
impl WinRtBleGateway for FakeGateway {
...
#[tokio::test]
async fn access_denied_returns_permission_required_without_scanning() {
    let gateway = FakeGateway {
        availability: WinRtBleAvailability::AccessDenied,
        ..Default::default()
    };
    let mut controller = WinRtBleCentralController::new(gateway);
...
async fn unavailable_adapter_returns_adapter_unavailable_without_scanning() {
    let gateway = FakeGateway {
        availability: WinRtBleAvailability::Unsupported,
        ..Default::default()
    };
    let mut controller = WinRtBleCentralController::new(gateway);
...
async fn powered_on_scan_uses_sms_pusher_service_uuid() {
    let gateway = FakeGateway {
        availability: WinRtBleAvailability::PoweredOn,
        ..Default::default()
    };
    let mut controller = WinRtBleCentralController::new(gateway);
